...
async def main():
    ...
    # 2. Test list_strings with Optimization
    client = SimpleSseClient(mcp_url)
    
    try:
        connected = await client.connect()
        if connected:
            # Test 1: list_strings with filter "shopee" (should trigger optimization)
            targets = "shopee"
            print(f"\nTest 1: list_strings(filter='{targets}') - Should trigger optimization")
            start_time = time.time()
            # The tool name is actually "mcp_ida-mcp-tool_strings" (based on api_core.py function name 'strings')
            # But usually tool names are derived from function names.
            # Let's try 'strings' first as defined in api_core.py
            res = await client.call_tool(mcp_base, "strings", {
                "queries": [{"filter": targets, "count": 20}]
            })
                        ...
                        try:
                            ...
                            if isinstance(content, list):
                                for item in content:
                                    ...
                                    print(f"Found: {len(items)} matches (Total est: {total})")
                                    if len(items) > 0:
                                        print(f"Matches: {items[:3]} ...")
                            else:
