...
async def main():
    ...
    try:
        ...
        if connected:
            # Test: Searching for 'shopee'
            targets = ["shopee"]
            print(f"\nSearching for {targets}")
            start_time = time.time()
            res = await client.call_tool(mcp_base, "search", {
                "type": "string",
                "targets": targets,
                "timeout": 60
            })
                        ...
                        try:
                            ...
                            if isinstance(content, list):
                                for item in content:
                                    ...
                                    print(f"Query: {query}, Found: {count} matches")
                                    if count > 0:
                                        print(f"Matches: {item['matches'][:10]} ...")
                            else:
                                count = content.get("count", 0)
                                print(f"Success! Found {count} matches.")
                                if count > 0:
                                    print(f"Matches: {content['matches'][:10]} ...")
                        except json.JSONDecodeError:
